...
def verify_paystack_payment(reference: str) -> dict[str, Any]:
    ...
    # Require at least the expected amount (dis should allow small overpayment due to fees)
    ...
    # Log the metadata so I can see it in your server logs too JIC
...
def _process_report(job_id: str, request: ReportRequest) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="giscopo-") as temp_dir:
            # Note to self ONLY CALL THIS ONCE
...
def generate_report(payload: ReportRequest, background_tasks: BackgroundTasks) -> dict[str, str]:
    ...
    # Bypass Paystack during local backend testing, UNPRECEDENTED ACCESSSSSS
...
# Dis is just to point the standard OpenAI client to the free GitHub Models endpoint
client = OpenAI(
    base_url="https://models.inference.ai.azure.com",
